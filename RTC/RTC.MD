...
static bool example_timer_on_alarm_cb(gptimer_handle_t timer, const gptimer_alarm_event_data_t *edata, void *user_ctx)
{
    ...
    // 可选：通过操作系统队列将事件数据发送给其他任务
    // 不要在回调函数中引入复杂逻辑
    // 建议在主循环中处理事件数据，而不是在这个回调中
    xQueueSendFromISR(queue, &ele, &high_task_awoken);
    // 返回我们是否需要在 ISR 结束时进行任务切换
    return high_task_awoken == pdTRUE;
}

gptimer_alarm_config_t alarm_config = {
    .reload_count = 0, // 报警事件发生时计数器将重新加载为 0
    .alarm_count = 1000000, // 周期 = 1秒 @分辨率 1MHz
    .flags.auto_reload_on_alarm = true, // 启用自动重载
};
ESP_ERROR_CHECK(gptimer_set_alarm_action(gptimer, &alarm_config));

gptimer_event_callbacks_t cbs = {
    .on_alarm = example_timer_on_alarm_cb, // 注册用户回调
};
...
gptimer_alarm_config_t alarm_config = {
    .alarm_count = 1 * 1000 * 1000, // 报警目标 = 1秒 @分辨率 1MHz
};
ESP_ERROR_CHECK(gptimer_set_alarm_action(gptimer, &alarm_config));
...
static bool example_timer_on_alarm_cb(gptimer_handle_t timer, const gptimer_alarm_event_data_t *edata, void *user_ctx)
{
    ...
    // 重新配置报警值
    gptimer_alarm_config_t alarm_config = {
        .alarm_count = edata->alarm_value + 1000000, // 下次报警在1秒后
    };
...
gptimer_alarm_config_t alarm_config = {
    .alarm_count = 1000000, // 初始报警目标 = 1秒 @分辨率 1MHz
};
ESP_ERROR_CHECK(gptimer_set_alarm_action(gptimer, &alarm_config));
